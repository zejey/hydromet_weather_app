...
class _TipsScreenState extends State<TipsScreen>
  ...
  void initState() {
    super.initState();
    // Start with a default length, will be updated when categories load
    _tabController = TabController(length: 1, vsync: this);
    _loadWeatherData();
    _loadCategories();
  }

  // 🔧 Fixed category loading method
  void _loadCategories() {
    _safetyTipsService.getActiveCategories().listen((loadedCategories) {
      if (mounted && loadedCategories.isNotEmpty) {
        setState(() {
          categories = loadedCategories;

          // 🔧 Safe TabController update
          if (_isInitialLoad || _tabController.length != categories.length) {
            // Only dispose if not the initial load
            if (!_isInitialLoad) {
              _tabController.dispose();
            }

            // Create new TabController with correct length
            _tabController =
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              // Tips Content with Dynamic Tabbed Interface
              Flexible(
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Weather-specific tip banner (keep the same)
                      if (weatherData != null)
                        Container(
                          width: double.infinity,
                          ...
                          child: TabBarView(
                            ...
                            children: categories.map((category) {
                              return SingleChildScrollView(
                                ...
                                child: _buildDynamicCategorySection(category),
                              );
                            }).toList(),
                          ),
                        ),
  ...
  // 🆕 Dynamic colored tab builder
  Widget _buildDynamicColoredTab({
  ...
  Widget _buildDynamicCategorySection(SafetyCategory category) {
    return Column(
      ...
      children: [
        ...
        StreamBuilder<List<SafetyTip>>(
          ...
          builder: (context, snapshot) {
            ...
            return Column(
              children: tips
                  .map((tip) => _buildDynamicColorCodedItem(
                        tip.range,
                        tip.level,
                        tip.description,
                        tip.colorData,
        ...
        // 🆕 Dynamic preventive measures
        StreamBuilder<List<PreventiveMeasure>>(
          stream:
              _safetyTipsService.getPreventiveMeasuresForCategory(category.id),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              return Text(
                  'Error loading preventive measures: ${snapshot.error}');
            }
            ...
            return Column(
              ...
              children: [
                ...
                ...measures
                    .map((measure) => _buildDynamicNumberedItem(
                          measure.number,
                          measure.title,
                          measure.description,
                        ))
  ...
  // 🆕 Dynamic color-coded item (same as before, but renamed)
  Widget _buildDynamicColorCodedItem(
  ...
  // 🆕 Dynamic numbered item (same as before, but renamed)
  Widget _buildDynamicNumberedItem(
  ...
  //   String title = type == 'AIR QUALITY'
  //       ? 'PREVENTIVE MEASURES'
  //       : type == 'HEAT'
  //           ? 'How to Overcome/PREVENTION'
  //           : type == 'FLOOD'
  //               ? 'PREVENTIVE MEASURES'
  //               : 'PREVENTIVE MEASURES';
  ...
  //       ),
  //       const SizedBox(height: 8),
  //       ...preventiveMeasures[type]!
  //           .map((item) => _buildNumberedItem(
  //                 item['number']!,
  //                 item['title']!,
  //                 item['description']!,
  //               ))
